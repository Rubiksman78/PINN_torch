...
def plot_training_points(t_0, t_b, t_r, x_0, x_b, x_r, y_0, y_b, y_r, u_0, u_b):
    """
    ...
    ax.scatter(t_0, x_0[:, 0], y_0[:, 0], c=u_0, marker='X', vmin=-1, vmax=1)
    ax.scatter(t_b, x_b[:, 0], y_b[:, 0], c=u_b, marker='X', vmin=-1, vmax=1)
    ax.scatter(t_r, x_r[:, 0], y_r[:, 0], c='r', marker='.', alpha=0.1)
    ax.set_xlabel('$t$')
    ax.set_ylabel('$x1$')
    ax.set_zlabel('$y1$')
...
def plot1dgrid_real(lb, ub, N, model, k, with_rnn=False):
    ...
    T, X1, Y1 = np.meshgrid(tspace, x1space, y1space)
    ...
    T, X1 = T.view(N, N, N).detach().cpu().numpy(), X1.view(
        N, N, N).detach().cpu().numpy()
    Y1 = Y1.view(N, N, N).detach().cpu().numpy()
...
def train(model, train_data, val_data, train_data_begin,
    ...
    for epoch in epochs:
        ...
        index_shuf_b = torch.randperm(train_data[3].shape[0])
        index_shuf_i = torch.randperm(train_data[5].shape[0])
        x_r_train = train_data[0][index_shuf_r].to(device)
        y_r_train = train_data[1][index_shuf_r].to(device)
        t_r_train = train_data[2][index_shuf_r].to(device)
        u_b_train = train_data[3][index_shuf_b].to(device)
        x_b_train = train_data[4][index_shuf_b].to(device)
        y_b_train = train_data[5][index_shuf_b].to(device)
        t_b_train = train_data[6][index_shuf_b].to(device)
        u_i_train = train_data[7][index_shuf_i].to(device)
        x_i_train = train_data[8][index_shuf_i].to(device)
        y_i_train = train_data[9][index_shuf_i].to(device)
        t_i_train = train_data[10][index_shuf_i].to(device)
        train_data_new = [x_r_train, y_r_train, t_r_train, u_b_train,
                          x_b_train, y_b_train, t_b_train, u_i_train, x_i_train, y_i_train, t_i_train]
        ...
        y_b_train = train_data_begin[2][index_shuf_b].to(device)
        t_b_train = train_data_begin[0][index_shuf_b].to(device)

        train_data_begin = [t_b_train, x_b_train, y_b_train]
        train_data = train_data_new
        if epoch < 1000:
            loss_begin = model.train_step(train_data_begin, phase="beginning")
            epochs.set_postfix(loss=loss_begin)
        else:
            loss_residual, loss_bords, loss_init, loss_bords_der, loss_trunc = model.train_step(
                train_data)
...
if __name__ == '__main__':
    ...
    t_ri, x_ri, y_ri = define_points_begin(10000, l_b, u_b)
    t_i, x_i, y_i, u_i, t_b, x_b, y_b, u_b, t_r, x_r, y_r = define_points(
        N_i, N_b, N_r, l_b, u_b)
    ...
    train_data, val_data = val_split(
        x_r, y_r, t_r, u_b, x_b, y_b, t_b, u_i, x_i, y_i, t_i, split=0.1)

    train_data_begin = [t_ri.to(device), x_ri.to(device), y_ri.to(device)]
    lb = [0, 0]
    ub = [1, 1]
